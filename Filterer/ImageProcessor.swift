...
class ImageProcessor: NSObject {
    fileprivate var image: RGBAImage
    ...
    func applyFilter(_ filter: [Double]) -> UIImage? {
        var filter = filter
    ...
    func saturation(_ intensity: Int) -> UIImage? {
    ...
    func findAndApplyFilter(_ filter: String) -> UIImage? {
        if filter == "Grey" {
            return greyScale()
        }//if
            ...
            for (filterName, filterMatrix) in filters {
                if filter == filterName {
                    return applyFilter(filterMatrix as! [Double])
                }//if
            }//for
        }//else
        return nil
    ...
    fileprivate func filterIsApplicable(_ index: Int) -> Bool {
    ...
    fileprivate func computeNeighbourPixels(_ index: Int) -> [Int] {
    ...
    private func normaliseMatrix(_ matrix: [Double]) -> [Double] {
        var matrix = matrix
        if matrix.count == 9 {
            return matrix
        }//if
